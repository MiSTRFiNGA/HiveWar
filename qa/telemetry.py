...
def one_run(browser, base_url, index, cap_sec, speed):
    ...
    page.goto(base_url + f"?telemetry=1&telemetrySpeed={speed}", wait_until="load", timeout=20_000)
    elapsed = 0.0
    ...
    try:
        while elapsed < cap_sec:
            ...
            if state.get("state") in ("dead", "win"):
                return {"run": index, "sim_sec": round(elapsed, 1), "state": state["state"],
                        "level": state.get("level"), "credits": state.get("credits"),
                        "first_shop_credits": first_shop_credits, "kills": state.get("kills"),
                        "errors": errors}
        return {"run": index, "sim_sec": cap_sec, "state": "TIMEOUT", "level": state.get("level"),
...
def main():
    ...
    with sync_playwright() as playwright:
        ...
        rows = [one_run(browser, args.url, i + 1, args.cap_sec, args.speed) for i in range(args.runs)]
        browser.close()
